...
val example1 = """Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
...
fun parseGame(s: String) = Regex("""Game (\d+): (.+)""").matchEntire(s)!!.let {
    ...
    Game(game.toInt(), moves.split("; ").map {
        Regex("""(\d+) (blue|red|green)""").findAll(it).map {
...
fun main() {
    ...
    val lines = object {}.javaClass.getResource("/day02.input")!!.readText().lines()
